...
class BinanceFuturesSource(SourceAdapter):
    ...
    def ohlcv_available_from_ms(self, symbol: str, timeframe: str) -> tuple[int | None, bool]:
        ...
        try:
            ...
            logger.exception("Failed to estimate Binance OHLCV availability for %s %s", symbol, timeframe)
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str, start_ms: int, end_ms: int) -> pd.DataFrame:
        ...
        while since < end_ms:
            batch = self._exchange.fetch_ohlcv(symbol, timeframe=timeframe, since=since, limit=1500)
            if not batch:
                break
